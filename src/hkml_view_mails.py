...
def mail_item_of_row(slist, row):
...
def get_focused_mail_item(slist):
    return mail_item_of_row(slist, slist.focus_row)
...
def open_focused_mail(c, slist):
    ...
    text_view_list = hkml_view_text.show_text_viewer(
            slist.screen,
            args=hkml_view_text.ShowTextViewerArgs(
                text_lines=lines, text_view_data=text_view_data,
...
def refresh_list(slist, show_tagged_mails):
    ...
    slist.data.list_data = hkml_list.MailsListData(
            text, len(comment_lines), mail_items=mail_items,
